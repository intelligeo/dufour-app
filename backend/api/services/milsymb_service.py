...
def get_milsymb_geojson(project_name: str, layer_title: str) -> Optional[Dict[str, Any]]:
    ...
    Strategy: try PostGIS first (fast), fall back to re-parsing .qgz (safe).
    ...
    """
    # ── 1. Try PostGIS (populated during upload migration) ────────────
    try:
        from services.milsymb_migrator import read_milsymb_geojson_from_postgis
        result = read_milsymb_geojson_from_postgis(project_name, layer_title)
        if result is not None:
            logger.info(
                f"Serving milsymb '{layer_title}' from PostGIS for '{project_name}'"
            )
            return result
    except Exception as e:
        logger.debug(f"PostGIS milsymb lookup failed (will try .qgz): {e}")

    # ── 2. Fallback: re-parse .qgz from DB ───────────────────────────
